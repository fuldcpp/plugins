# Bygger Babelfish-x64.dll. Hittar Visual Studio sjalv via vswhere, sa bygget
...
$dll = Join-Path $build 'Release\Babelfish-x64.dll'
if (Test-Path $dll) {
...
# Testharnesken byggs med samma kommando; kor den direkt sa att ett trasigt
# filter aldrig hinner bli en DLL nagon installerar.
$test = Join-Path $build 'Release\translatetest.exe'
if (Test-Path $test) {
    & $test
    if ($LASTEXITCODE -ne 0) { throw "Testerna misslyckades" }
}
